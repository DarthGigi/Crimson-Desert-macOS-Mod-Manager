<script lang="ts">
	...
	async function chooseBinaryTarget() {
		const selected = await open({ multiple: false, directory: false, defaultPath: binaryTargetPath || undefined, title: 'Choose the source file to patch' });
		if (typeof selected === 'string') binaryTargetPath = selected;
	}

	async function chooseBinaryOutput() {
		const selected = await open({ multiple: false, directory: false, defaultPath: binaryOutputPath || undefined, title: 'Choose the output file for the patched result' });
		if (typeof selected === 'string') binaryOutputPath = selected;
	}

	async function chooseScriptWorkingDir() {
		const selected = await open({ multiple: false, directory: true, defaultPath: scriptWorkingDir || undefined, title: 'Choose a working directory for the script installer' });
...
<div class="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
	<div class="space-y-2">
		<p class="text-xs font-medium tracking-[0.24em] text-muted-foreground uppercase">External Mods</p>
	...
	<Card.Root>
		<Card.Header>
			<Card.Title class="flex items-center gap-2"><Archive class="size-5" /> Imported external mods</Card.Title>
			...
			{#if manager.bnkMods.length === 0 && manager.scriptMods.length === 0 && manager.binaryPatchMods.length === 0}
				<Empty.Root class="min-h-40 border-dashed bg-muted/20 p-8">
					<Empty.Header>
						...
						<Empty.Description>Import BNK, script-installer, or binary patch mod sources from the Data Mods page.</Empty.Description>
					</Empty.Header>
				</Empty.Root>
			{:else}
				<div class="space-y-3">
	...
	<Card.Root>
			<Card.Header>
				...
				<Card.Description>BNK files copied into the managed soundbank target directory for this install.</Card.Description>
			</Card.Header>
			<Card.Content>
				...
				{:else}
					<div class="space-y-2">
						{#each manager.bnkFiles as file (file.path)}
							<div class="flex flex-col gap-2 rounded-lg border bg-background/60 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
								<div>
									<p class="font-medium">{file.name}</p>
									<p class="text-xs text-muted-foreground break-all">{file.path}</p>
								</div>
								<Button variant="destructive" size="sm" onclick={() => manager.removeBnkFile(file.name)}>Remove</Button>
							</div>
						{/each}
					</div>
				{/if}
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>Pick a source file and an output file, then apply an imported `.bsdiff` or `.xdelta` patch mod.</Card.Description>
		</Card.Header>
		<Card.Content class="space-y-4">
			<div class="space-y-2">
				<Label for="binary-target">Source file</Label>
				<div class="flex flex-wrap gap-2">
					<Input id="binary-target" bind:value={binaryTargetPath} placeholder="Choose the file to patch" />
			...
			<div class="space-y-2">
				...
				<div class="flex flex-wrap gap-2">
					<Input id="binary-output" bind:value={binaryOutputPath} placeholder="Choose where to write the patched result" />
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>Stage the script files, then run the imported installer with a chosen working directory.</Card.Description>
		</Card.Header>
		<Card.Content class="space-y-4">
			<div class="space-y-2">
				<Label for="script-working-dir">Working directory</Label>
				<div class="flex flex-wrap gap-2">
					<Input id="script-working-dir" bind:value={scriptWorkingDir} placeholder="Choose a working directory" />
