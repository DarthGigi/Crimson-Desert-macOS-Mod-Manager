...
<div class="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
		...
		<Card.Content class="space-y-4">
			<div class="space-y-2">
				...
				<div class="flex flex-wrap gap-2">
					...
					<Button variant="outline" disabled={manager.busy.pathc} onclick={() => manager.refreshPathcSummary(pathcPathInput, pathcLookupInput)}>
			...
			<div class="space-y-2">
				...
				<div class="flex flex-wrap gap-2">
					...
					<Button disabled={manager.busy.repackingPathc} onclick={() => manager.runPathcRepack(pathcPathInput, pathcFolderInput)}>
			...
			{#if manager.pathcResult}
				<Alert.Root>
					...
					<Alert.Description>
						Processed {manager.pathcResult.processedCount} DDS file(s), updated {manager.pathcResult.updatedCount} hash entries, added {manager.pathcResult.addedTemplateCount} new DDS templates. Backup: {manager.pathcResult.backupPath}
					</Alert.Description>
				</Alert.Root>
			{/if}
		</Card.Content>
	...
	<Card.Root>
		...
		<Card.Content class="space-y-4">
			...
			<div class="flex flex-wrap gap-2">
				<Button variant="outline" disabled={manager.busy.extracting} onclick={() => manager.refreshExtractPreview(extractVirtualPathInput, extractSourceGroupInput || null)}>
					{manager.busy.extracting ? 'Inspecting...' : 'Inspect file'}
				</Button>
				<Button disabled={manager.busy.extracting} onclick={() => manager.runExtractVirtualFile(extractVirtualPathInput, extractSourceGroupInput || null, extractOutputDirInput)}>
			...
			{#if manager.extractPreview}
				<div class="rounded-xl border bg-muted/20 p-4 text-sm">
					<div class="flex flex-wrap items-center gap-2">
						...
						<Badge variant={manager.extractPreview.resolved ? 'outline' : 'secondary'}>{manager.extractPreview.resolved ? 'Resolved' : 'Missing'}</Badge>
					</div>
					<p class="text-muted-foreground mt-2 text-xs">Source group: {manager.extractPreview.sourceGroup}</p>
					{#if manager.extractPreview.resolved}
						<p class="text-muted-foreground mt-1 text-xs">
							{manager.extractPreview.resolvedGameFile} / PAZ {manager.extractPreview.sourcePazIndex} / {manager.extractPreview.compressedSize} compressed / {manager.extractPreview.decompressedSize} decompressed / flags {manager.extractPreview.flags}
						</p>
					{:else if manager.extractPreview.reason}
						<p class="text-destructive mt-1 text-xs">{manager.extractPreview.reason}</p>
					{/if}
				</div>
			{/if}
		...
		<Card.Content class="space-y-4">
			<div class="grid gap-4 sm:grid-cols-2">
				<div class="space-y-2 sm:col-span-2">
					...
					<Input id="virtual-search" bind:value={virtualSearchInput} placeholder="ui/texture or playeractiongraph" />
				</div>
				<div class="space-y-2">
					<Label for="search-source-group">Source group filter</Label>
					<Input id="search-source-group" bind:value={extractSourceGroupInput} placeholder="0008 or leave blank" />
				</div>
				<div class="flex items-end">
					<Button variant="outline" disabled={manager.busy.searchingFiles} onclick={() => manager.searchVirtualFiles(virtualSearchInput, extractSourceGroupInput || null, 100)}>
			...
			<ScrollArea.Root class="h-64 rounded-xl border">
				<div class="divide-y">
					{#each manager.virtualFileMatches as match (match.sourceGroup + match.virtualPath)}
						<button
							class="hover:bg-muted/40 w-full space-y-1 px-4 py-3 text-left transition"
							onclick={async () => {
								extractVirtualPathInput = match.virtualPath;
								extractSourceGroupInput = match.sourceGroup;
								await manager.refreshExtractPreview(extractVirtualPathInput, extractSourceGroupInput);
							}}
						>
							...
							<p class="text-xs text-muted-foreground">
								PAZ {match.sourcePazIndex} / {match.compressedSize} compressed / {match.decompressedSize} decompressed / flags {match.flags}
							</p>
						</button>
					{:else}
						<div class="px-4 py-8 text-sm text-muted-foreground">No virtual file matches loaded yet.</div>
					{/each}
				</div>
			</ScrollArea.Root>
		</Card.Content>
