...
<div class="mx-auto flex min-h-full w-full max-w-5xl flex-col gap-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
	...
	<Card.Root>
		<Card.Header>
			<Card.Title class="flex items-center gap-2"><Sparkles class="size-5" /> Create profile</Card.Title>
		...
		<Card.Content class="space-y-4">
			...
			<div class="space-y-2">
				...
				<Input id="profile-description" bind:value={description} placeholder="Optional note about this mod set" />
			</div>
			<Button onclick={() => manager.createProfile(name, description || null)}>Create from current enabled mods</Button>
		</Card.Content>
	</Card.Root>
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description>Apply a profile to replace the current enabled set, or resave it from current state.</Card.Description>
		</Card.Header>
		<Card.Content>
			{#if manager.profiles.length === 0}
				<Empty.Root class="min-h-40 border-dashed bg-muted/20 p-8">
					<Empty.Header>
						...
						<Empty.Description>Create your first profile from the currently enabled mods.</Empty.Description>
					</Empty.Header>
				</Empty.Root>
			{:else}
				<div class="space-y-3">
						...
						<div class="rounded-xl border bg-muted/20 px-4 py-4">
							<div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
								<div>
									...
									{#if profile.description}<p class="text-sm text-muted-foreground">{profile.description}</p>{/if}
								</div>
								<div class="flex flex-wrap gap-2">
									<Button size="sm" onclick={() => manager.applyProfile(profile.id)}>Apply</Button>
									<Button variant="outline" size="sm" onclick={() => manager.saveProfile(profile.id)}>Resave</Button>
									<Button variant="destructive" size="sm" onclick={() => manager.deleteProfile(profile.id)}>Delete</Button>
								</div>
							</div>
						</div>
					{/each}
